...
import type { WeeklyHours } from "@/lib/store/hours";
...
export function getTodayClosingMinutes(hours: WeeklyHours, at: Date = new Date()): number | null {
...
export function getLastOrderDeadline(hours: WeeklyHours, at: Date = new Date()): Date | null {
  const closingMinutes = getTodayClosingMinutes(hours, at);
  if (closingMinutes === null) return null;

  const deadline = new Date(at);
  deadline.setHours(0, 0, 0, 0);
  deadline.setMinutes(closingMinutes - LAST_ORDER_MINUTES_BEFORE_CLOSE);

  return deadline;
}

export function getLateOrderWindowStart(hours: WeeklyHours, at: Date = new Date()): Date | null {
  const lastOrder = getLastOrderDeadline(hours, at);
  if (!lastOrder) return null;
  return new Date(lastOrder.getTime() - LATE_ORDER_WINDOW_MINUTES * 60_000);
}
...
export function validateOrderForClosing(
  ...
  if (!hoursStr || hoursStr.toLowerCase() === "closed") {
    throw new ClosingValidationError("We're closed today — we're not taking orders.");
  }
  ...
  const lateWindowStart = getLateOrderWindowStart(hours, at);
  ...
  for (const input of items) {
    ...
    if (inLateWindow && menuItem.prep_time_minutes > MAX_PREP_DURING_LATE_WINDOW) {
      throw new ClosingValidationError(
  ...
  const { ready_by } = calculateReadyBy(items, menuItems, orderSize);
